...
class DroneController:
    def __init__(self):
        ...
        self.drone.speed = 25
         ...
         # Variabel status joystick
        self.joystick_active = True  # Aktifkan joystick kontrol
    ...
    def process_joystick_input(self):
        ...
        # Perbarui event joystick
        pygame.event.pump()

        # Ambil nilai tuas (axes)
        left_stick_x = self.joystick.get_axis(0)  # Horizontal tuas kiri
        left_stick_y = self.joystick.get_axis(1)  # Vertikal tuas kiri
        right_stick_x = self.joystick.get_axis(2)  # Horizontal tuas kanan
        right_stick_y = self.joystick.get_axis(3)  # Vertikal tuas kanan

        # Menerjemahkan nilai tuas menjadi kontrol drone
        if abs(left_stick_y) > 0.1:
            if left_stick_y < 0:
                self.drone.move_up(int(self.drone.speed * abs(left_stick_y)))
            else:
                self.drone.move_down(int(self.drone.speed * abs(left_stick_y)))

        if abs(right_stick_y) > 0.1:
            if right_stick_y < 0:
                self.drone.move_forward(int(self.drone.speed * abs(right_stick_y)))
            else:
                self.drone.move_back(int(self.drone.speed * abs(right_stick_y)))

        if abs(left_stick_x) > 0.1:
            if left_stick_x < 0:
                self.drone.move_left(int(self.drone.speed * abs(left_stick_x)))
            else:
                self.drone.move_right(int(self.drone.speed * abs(left_stick_x)))

        if abs(right_stick_x) > 0.1:
            if right_stick_x < 0:
                self.drone.rotate_counter_clockwise(int(self.drone.speed * abs(right_stick_x)))
            else:
                self.drone.rotate_clockwise(int(self.drone.speed * abs(right_stick_x)))

        # Tombol untuk takeoff dan landing
        if self.joystick.get_button(0):  # Tombol A
            self.drone.takeoff()
        elif self.joystick.get_button(1):  # Tombol B
            self.drone.land()
